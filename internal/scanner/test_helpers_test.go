...
func setupTestDB(t *testing.T) (*sql.DB, database.Querier) {
	...
	// We need to navigate up from internal/scanner to sql/schema.
...
func setupLogicTest(t *testing.T) (*sql.DB, database.Querier, *Scanner, string) {
	...
	scanner := NewScanner(conn, queries, mockThumbGen, logger, notifier, cfg)
	cfg.SetAllowedExtensions([]string{".txt", ".png"}) // Set simple extensions for testing
	return conn, queries, scanner, root
}

// createDummyFile helper creates a file with dummy content at the specified path.
...
// createContentFile helper creates a file with specific content.
