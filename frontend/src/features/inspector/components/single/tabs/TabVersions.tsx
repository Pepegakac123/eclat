...
export const TabVersions = ({ asset }: TabVersionsProps) => {
  ...
  const { duplicates, timeline } = useMemo(() => {
    if (!siblings) return { duplicates: [] as app.AssetDetails[], timeline: [] as app.AssetDetails[] };

    // Filter out the current asset itself
  ...
  const handleDeleteDuplicate = async (dupId: number) => {
    if (!window.confirm("Are you sure you want to delete this duplicate file permanently?")) return;
    
    setIsDeletingDupId(dupId);
    try {
      await DeleteAssetsPermanently([dupId]);
      addToast({
        title: "Deleted",
      ...
      <div className="flex flex-col items-center justify-center py-12 gap-3 text-default-400">
        ...
        <div className="text-center">
          ...
          <p className="text-[10px] opacity-60 max-w-[180px]">
            Eclat automatically groups similar files by name or content.
          </p>
        </div>
      </div>
    );
                ...
                {duplicates.map((dup: app.AssetDetails) => (
                  ...
                  >
                    <div className="flex flex-col gap-0.5 overflow-hidden">
                      <Tooltip content={dup.filePath} placement="top-start" delay={500} closeDelay={0}>
          ...
          <div className="relative ml-3.5 pl-6 border-l-2 border-default-100 flex flex-col gap-4">
            ...
            <div className="relative">
              ...
              <div className="flex flex-col gap-1 p-2 rounded-lg bg-primary/5 border border-primary/10">
                <div className="flex items-center justify-between">
                  <span className="text-tiny font-bold text-primary italic">Active Version</span>
                  <Chip size="sm" variant="flat" color="primary" className="h-4 text-[8px] uppercase">Current</Chip>
                </div>
                <span className="text-tiny text-default-900 truncate font-medium">{asset.fileName}</span>
                ...
                <div className="flex flex-col gap-1 p-2 rounded-lg bg-default-50 border border-default-100 group-hover:border-default-200 transition-all cursor-pointer">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Tooltip content="Switch to this version">
                       <ArrowRight size={10} className="text-default-300 opacity-0 group-hover:opacity-100 transition-opacity" />
                    </Tooltip>
                  </div>
                </div>
              </div>
            ...
            {timeline.length === 0 && (
              <div className="py-2 text-[10px] text-default-400 italic">
                No historical versions found in this group.
              </div>
            )}
          </div>
        </div>
