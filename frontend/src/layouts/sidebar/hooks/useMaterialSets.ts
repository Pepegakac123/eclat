...
export const useMaterialSets = () => {
	...
	const createMutation = useMutation({
		...
		onError: (error: any) => {
			addToast({
				...
				description: error.message || "Could not create material set.",
				color: "danger",
				severity: "danger",
				variant: "flat",
			});
	...
	const updateMutation = useMutation({
		mutationFn: ({
			...
			id: number;
			data: app.CreateMaterialSetRequest;
		}) => Update(id, data),
		...
		onError: (error: any) => {
			addToast({
				title: "Update Failed",
				description: error.message || "Could not update material set.",
				color: "danger",
				severity: "danger",
				variant: "flat",
			});
	...
	const deleteMutation = useMutation({
		...
		onError: (error: any) => {
			addToast({
				title: "Delete Failed",
				description: error.message || "Could not delete material set.",
				color: "danger",
				severity: "danger",
				variant: "flat",
			});
	...
	const setCoverFromFileMutation = useMutation({
		mutationFn: ({ id, filePath }: { id: number; filePath: string }) =>
			SetMaterialSetCoverFromFile(id, filePath),
		onSuccess: (_, variables) => {
			queryClient.invalidateQueries({ queryKey: QUERY_KEY });
			queryClient.invalidateQueries({
				queryKey: ["material-set", variables.id],
			...
			addToast({
				title: "Cover Updated",
				description: "Collection cover has been set from file.",
				color: "success",
				variant: "flat",
			});
		},
		onError: (error: any) => {
			addToast({
				title: "Update Failed",
				description: error.message || "Could not set cover image.",
				color: "danger",
				variant: "flat",
			});
		},
	...
	return {
		...
		setCoverFromFile: setCoverFromFileMutation.mutateAsync,
		isSettingCover: setCoverFromFileMutation.isPending,
